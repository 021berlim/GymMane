  ...
  static Future<void> _push() async {
    ...
    if (paused) {
      ...
      if (pending < 0) {
        ...
        final load = '${st.reps} × ${fit.weightLabel(st.weight)}';
    ...
    final key = [
      ...
      index,
      total,
      resting ? restEnd.millisecondsSinceEpoch : 0,
      paused,
      position,
      actions.length,
      finished.join(),
      _flashes,
      paused ? fit.sessionElapsed : 0,
    ].join('|');
